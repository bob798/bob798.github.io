...
写完代码不算完，还要 review。我用 [oh-my-claudecode (OMC)](https://github.com/anthropics/claude-code) 的多智能体能力对这套实现做了审查，这里分享一下 OMC 的 review 体系，写完代码可以直接用。

### `/simplify` — 快速审查已改动的代码

这是最轻量的方式。执行 `/simplify`，OMC 会自动扫描最近修改的代码，检查复用性、质量和效率问题并直接修复。适合写完代码后的第一遍自查。

### `code-reviewer` — 深度代码审查

需要更严格的 review 时，用 `code-reviewer` agent（opus 模型驱动）：
...
比如在这个项目中，`code-reviewer` 指出了图片提取通过 `rels` 遍历会遗漏 VML 嵌入图片的问题，也就是踩坑记录第 1 条。

### `security-reviewer` — 安全审查

处理用户上传的 Word 文档时，安全问题不能忽视：

```
用 security-reviewer 检查这个项目的安全性
```

它专注 OWASP Top 10、敏感信息泄露和不安全模式。对于文件解析场景，会检查 zip 炸弹防护、路径穿越、临时文件清理等问题。

### `critic` — 方案设计评审

在动手写代码之前，可以先让 `critic` 评审你的设计方案：

```
用 critic 审查我的 RAG 系统设计方案
```

它会从多个视角挑战你的方案，找出架构盲点。比如"图片 OCR 结果放在文档最后会丢失位置信息"这个问题，就是 `critic` 在设计阶段指出的。

### 审查能力速查表

| 需求 | 用什么 | 模型 | 适合阶段 |
|------|--------|------|---------|
| 代码快速自查 | `/simplify` | sonnet | 写完代码后 |
| 深度代码审查 | `code-reviewer` | opus | 提交 PR 前 |
| 安全漏洞扫描 | `security-reviewer` | sonnet | 上线前 |
| 方案设计评审 | `critic` | opus | 动手写代码前 |
| GitHub PR 审查 | 直接贴 PR 链接 | 自动路由 | Code Review |

把 review 嵌入开发流程，而不是写完再补，代码质量会好很多。
...
写完代码后，用 OMC 的 `/simplify`、`code-reviewer`、`security-reviewer` 做多维度审查，确保代码质量和安全性。
